...
public class VideoRenderer implements GLWallpaperService.Renderer {
	...
	public VideoRenderer(VideoLiveWallpaper p, VideoLiveWallpaper.CubeEngine e) {
		super();
		mParent = p;
		mParentEngine = e;
	...
	public VideoRenderer(VideoLiveWallpaper p, VideoLiveWallpaper.CubeEngine e, int w, int h) {
		super();
		mParent = p;
		mParentEngine = e;
		videoWidth = w;
	...
	public void process(int width, int height) {
		...
		isScreenPortrait = (screenWidth > screenHeight) ? false : true;
		...
		runOnce = true;
		if (mParentEngine.isPreview())
			return;
		mParentEngine.onOffsetsChanged(VideoLiveWallpaper.xOffset, VideoLiveWallpaper.yOffset, VideoLiveWallpaper.xStep, VideoLiveWallpaper.yStep, 0, 0);
	}
	...
	public void onDrawFrame(GL10 gl) {
		synchronized (lock) {
			...
			if (MyDebug.showFPS) {
				...
				if (MyDebug.VR)
					Log.d(TAG, TAG + "drawFrame(): fps: " + String.valueOf(fpsRate));
			}
		}
	}
	...
	public void setScreenDimensions(int w, int h) {
		screenWidth = w;
		screenHeight = h;
		NativeCalls.setScreenDimensions(screenWidth, screenHeight);
		if (MyDebug.VR)
	...
	public void setWallVideoDimensions(int w, int h) {
		int[] newdims = scaleBasedOnHeight(w, h, screenWidth, screenHeight);
		wallVideoWidth = newdims[0];
		wallVideoHeight = newdims[1];
		NativeCalls.setWallVideoDimensions(wallVideoWidth, wallVideoHeight);
		if (MyDebug.VR)
	...
	public void setTextureDimensions(int width, int height) {
		...
		NativeCalls.setTextureDimensions(powWidth, powHeight);
		if (MyDebug.VR)
