...
public class VideoLiveWallpaper extends GLWallpaperService implements SharedPreferences.OnSharedPreferenceChangeListener {
	...
	public static final String defaultVideoName = "sdcard/a.flv";
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		...
		if (MyDebug.VLW)
			Log.d(TAG, "span video :" + spanVideo);
		// file name
	...
	class CubeEngine extends GLWallpaperService.GLEngine {
		...
		CubeEngine() {
			super();
			if (MyDebug.VLW)
				Log.d(TAG, "CubeEngine CubeEngine()");
			renderer = new VideoRenderer(VideoLiveWallpaper.this, this);
		...
		public void onSurfaceChanged(SurfaceHolder holder, int format, int w, int h) {
			if (MyDebug.VLW)
				Log.d(TAG, "CubeEngine onSurfaceChanged()");
		...
		public void onSurfaceCreated(SurfaceHolder holder) {
			if (MyDebug.VLW)
				Log.d(TAG, "CubeEngine onSurfaceCreated()");
		...
		public void onSurfaceDestroyed(SurfaceHolder holder) {
			if (MyDebug.VLW)
				Log.d(TAG, "CubeEngine onSurfaceDestroyed");
		...
		public void onOffsetsChanged(float xOffset, float yOffset, float xStep, float yStep, int xPixels, int yPixels) {
			...
			if (MyDebug.VLW)
				Log.d(TAG, "onoffsetschanged:" + String.valueOf(xOffset) + "," + String.valueOf(xStep) + "," + String.valueOf(xPixels));
		...
		public void calculateWallpaperDimensions() {
			...
			noScreensX = (Float.compare(xStep, 0.0f) == 0) ? 1 : ((int) (1.0f / xStep)) + 1;
			noScreensY = (Float.compare(yStep, 0.0f) == 0) ? 1 : ((int) (1.0f / yStep)) + 1;
		...
		public void mapXYoffsets() {
			...
			float adjustedWidth = (float) getRenderer().wallWidth - (float) getRenderer().wallWidth / (float) noScreensX;
			float adjustedHeight = (float) getRenderer().wallHeight - (float) getRenderer().wallHeight / (float) noScreensY;
		...
		public void onVisibilityChanged(boolean visible) {
			if (MyDebug.VLW)
				Log.d(TAG, "CubeEngine onVisibilityChanged()");
